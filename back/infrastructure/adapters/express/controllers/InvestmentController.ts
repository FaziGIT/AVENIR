...
export class InvestmentController {
                ...
                stocks.map(async stock => {
                    ...
                    if (trades.length > 0) {
                        ...
                        const referencePrice =
                            tradesBeforeCutoff.length > 0
                                ? [...tradesBeforeCutoff].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0]
                                      .price
    ...
    async getPortfolio(req: Request, res: Response) {
        try {
            ...
            try {
                ...
                const historyResponse = await this.getPortfolioHistory(mockRequest, mockResponse);
                ...
                if (history.length >= 2) {
                    ...
                    if (todayData && yesterdayData) {
                        ...
                        yesterdayIncomePercent =
                            yesterdayData.value > 0 ? (yesterdayIncome / yesterdayData.value) * 100 : 0;
                    }
    ...
    async getRecentTrades(req: Request<{}, {}, {}, { limit?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async purchaseStock(req: Request<{}, {}, { stockId: string; quantity: number }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async placeOrder(
        ...
        try {
            ...
            const userId = req.user.userId;
            const { stockId, side, type, quantity, limitPrice } = req.body;
            ...
            if (updatedOrder) {
                ...
                if (
                    type === OrderBookType.MARKET &&
                    updatedOrder.state === OrderBookState.CANCELLED &&
    ...
    async getOrderBook(req: Request<{ stockId: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async getStockTrades(req: Request<{ stockId: string }, {}, {}, { limit?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async getUserOrders(req: Request<{}, {}, {}, { stockId?: string; state?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async getStockPrices(req: Request<{ stockId: string }, {}, {}, { period?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
            ...
            const filteredTrades = startDate ? allTrades.filter(trade => trade.createdAt >= startDate) : allTrades;
    ...
    async getPortfolioHistory(req: Request<{}, {}, {}, { period?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    async getProfitsBreakdown(req: Request<{}, {}, {}, { period?: string }>, res: Response) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
            ...
            const breakdown = await Promise.all(
                portfolios.map(async portfolio => {
                    ...
                    if (!startDate) {
                        ...
                        const priceAtStart =
                            tradesInPeriod.length > 0 ? tradesInPeriod[0].price : portfolio.stock.currentPrice;
    ...
    async getAllStocksAdmin(req: Request, res: Response) {
        try {
    ...
    async createStock(req: Request, res: Response) {
        try {
            const { symbol, name, currentPrice, marketCap, isActive } = req.body as any;
    ...
    async updateStock(req: Request<{ id: string }>, res: Response) {
        try {
            const { id } = req.params;
            const { symbol, name, isin, currentPrice, marketCap, isActive } = req.body as any;
    ...
    async deleteStock(req: Request<{ id: string }>, res: Response) {
        try {
            const { id } = req.params;
            ...
            if (trades.length > 0) {
                return res.status(409).json({
                    error: 'Conflict',
                    message:
                        'Impossible de supprimer une action avec des transactions existantes. Désactivez-la à la place.',
                });
            }
