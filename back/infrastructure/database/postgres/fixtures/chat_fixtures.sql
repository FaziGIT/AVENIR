-- Fixtures pour le système de chat (PostgreSQL)
...
TRUNCATE messages, chats CASCADE;
...
SELECT
    ...
    NOW() - INTERVAL '1 hour'
FROM
    (SELECT id FROM users WHERE role = 'CLIENT' OFFSET 1 LIMIT 1) c,
    (SELECT id FROM users WHERE role = 'ADVISOR' OFFSET 1 LIMIT 1) a;
...
VALUES (
    'chat-closed-1',
    'client-john-doe', -- John Doe
    'adv-003', -- Sophie Dubois
    'CLOSED',
    NOW() - INTERVAL '7 days',
    NOW() - INTERVAL '5 days'
);
